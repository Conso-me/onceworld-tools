...
export function PhysicalOffensiveSummary({ rows, playerAtk, playerLuck }: Props) {
  ...
  return (
    <div className="bg-indigo-50 rounded-2xl px-4 py-3 border border-indigo-100">
      <div className="text-xs font-bold text-indigo-600 mb-2">物理攻撃 最大値サマリー</div>
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-white rounded-xl px-3 py-2 col-span-1">
          ...
          {hardestRow.dmg?.isNullified ? (
            <div className="text-sm font-semibold text-gray-400">無効化</div>
          ) : (
          ...
          <div className="mt-1">
            <div className="text-[10px] text-gray-400">ダメージを通すATK</div>
            ...
            {playerAtk > 0 && (
              <div className={`text-xs tabular-nums font-medium ${atkAchieved ? "text-green-600" : "text-orange-500"}`}>
                {atkAchieved ? "達成済み ✓" : `あと${atkShortfall.toLocaleString()}`}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl px-3 py-2 col-span-1">
          ...
          <div className="mt-1">
            <div className="text-[10px] text-gray-400">100%命中に必要なLUCK</div>
            ...
            {playerLuck > 0 && (
              <div className={`text-xs tabular-nums font-medium ${luckAchieved ? "text-green-600" : "text-orange-500"}`}>
                {luckAchieved ? "達成済み ✓" : `あと${luckShortfall.toLocaleString()}`}
              </div>
            )}
          </div>
        </div>
